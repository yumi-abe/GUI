...
class CrossTradeCaluculator(BusinessDayCalculator):
    ...
    def culculate_cross_trade(self, month:int, year:Optional[int]=None, start:Optional[Union[str, date]]=None,) -> Dict[str, date]:
        """
        クロス取引の日付を計算する
        :param month:対象月
        ...
        # クロス受け渡し日（約定日の2営業日後）
        if start != None:
            start_date = strToDate(self.add_business_days(start))
    ...
    def calculate_cross_fee(self, start:Union[str, date], end:Optional[Union[str, date]]=None) -> Dict[str, int]:
        """
        クロス取引の手数料計算
        :param start:開始日（取得日）
        :param end:終了日
...
cross_day = caluculator.culculate_cross_trade(12)
print(cross_day)
